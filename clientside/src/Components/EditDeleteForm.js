...
const EditDeleteForm = ({ onCancel }) => {
  const { id } = useParams(); // Assumes route like /edit/:id
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/api/destination/${id}`)
      .then(res => {
  ...
  const handleSubmit = e => {
    ...
    axios.put(`http://localhost:5000/api/destination/${id}`, formData)
      .then(() => {
